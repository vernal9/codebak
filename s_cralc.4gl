...
# Modify.........: No:CHI-C30128 13/05/23 By bart 製程成本改善
# -------------------------------- 改善內容 --------------------------------
# 1.工單展備料時如有作業編號，應帶入對應的製程段、製程序給sfa012/sfa013
# --------------------------------------------------------------------------
# Modify.........: No:MOD-E50001 14/10/13 By Mandy FUNCTION cralc_rate()內的p_total 改DEFINE num26_10,避免傳入被截斷,產生尾差問題
# Modify.........: No:MOD-F10112 15/01/26 By liliwen 若特性為1:不需發料/非必要,則來源特性為X:資訊參考
# Modify.........: No:MOD-F20024 15/02/09 By liliwen 續MOD-F10112,調整brb14製程BOM段落
# Modify.........: No:MOD-F20057 15/02/12 By liliwen 設定為3展開時,自己本身不需備料
# Modify.........: No:MOD-F70126 15/07/24 By catmoon 在展下一階時，應以工單的特性代碼(sfb95)為主，沒輸入再抓料件主特性代碼(ima910)
# Modify.........: No:MOD-F80119 15/08/21 By catmoon 工單型態:5 再加工工單，並且為事後倒扣料時(sfb39='2') 單身特性sfa11 = 'E' 
# Modify.........: No:FUN-FC0005 15/12/11 By fionchen 調整cralc_rate與cralc_eck_rate,增加傳入組成用量(bmb06)與主件底數(bmb07)兩個參數,
#                                                     且計算應發數量時,直接用組成用量與主件底數相除計算,而不是使用進位後的PQC計算 
...
DEFINE
    ...
    g_btflg      LIKE type_file.chr1,          # Prog. Version..: '5.30.24-17.04.13(01) #blow through flag
...
DEFINE
    ...
    p_btflg      LIKE type_file.chr1,          # Prog. Version..: '5.30.24-17.04.13(01) #blow through flag
    ...
    p_mps        LIKE type_file.chr1,          # Prog. Version..: '5.30.24-17.04.13(01) #if MPS phantom, blow through flag (Y/N)
    p_yld        LIKE type_file.chr1,          # Prog. Version..: '5.30.24-17.04.13(01) #inflate yield factor (Y/N)
...
#   LET g_sfa.sfa07 =0  #FUN-940008 mark
   ...
   #LET g_sfa.sfa11 ='N' #MOD-F80119 mark
   #MOD-F80119-add--start---------------- 
    ...
    WHILE TRUE
        ...
        FOREACH cralc_cur USING b_seq INTO sr[l_ac].*
          ...
          #------------------------------
            ...
            IF p_key2='' THEN                  #MOD-F70126 add
              #FUN-8B0015--BEGIN--
               LET l_ima910[l_ac]=''
               SELECT ima910 INTO l_ima910[l_ac] FROM ima_file WHERE ima01=sr[l_ac].bmb03
            ELSE                               #MOD-F70126 add
               LET l_ima910[l_ac] = p_key2     #MOD-F70126 add
            END IF                             #MOD-F70126 add
       ...
       #---------No.MOD-830161 end
        ...
        FOR l_i = 1 TO l_x
            ...
            END IF
           #CHI-C30128(1) -- modify start --
            #-->無製程序號
           #IF g_opseq IS NULL THEN LET g_opseq=' ' END IF
            LET l_sfa012 = ' ' 
            LET l_sfa013 = 0
            IF cl_null(g_opseq) THEN
               LET g_opseq = ' '
            ELSE
               SELECT ecm012,ecm03 INTO l_sfa012,l_sfa013 FROM ecm_file
                WHERE ecm01 = g_sfb.sfb01
                  AND ecm04 = g_opseq
            END IF
           #CHI-C30128(1) -- modify end --
...
#            LET l_total=sr[l_i].bmb06*p_total*((100+sr[l_i].bmb08))/100
          ...
          #CALL cralc_rate(p_key,sr[l_i].bmb03,p_total,sr[l_i].bmb081,sr[l_i].bmb08,sr[l_i].bmb082,l_bmb06,p_QPA) 
           #FUN-FC0005 mark end   ---------------------------------------
           #FUN-FC0005 add start ---------------------------------------
           LET l_bmb06_07=sr[l_i].bmb06/sr[l_i].bmb07                 
           CALL cralc_rate(p_key,sr[l_i].bmb03,p_total,sr[l_i].bmb081,sr[l_i].bmb08,sr[l_i].bmb082,l_bmb06_07,p_QPA,sr[l_i].bmb06,sr[l_i].bmb07) 
           #FUN-FC0005 add end   ---------------------------------------
                      ...
                      WHEN sr[l_i].bmb19='3'     #展開
                          ...
                          #CHI-8A0002-begin-add
                           ...
                           ELSE 
                             ...
                             #NO.FUN-A60080--end
                             #MOD-7B0075----mos---str---
                                 LET fs_insert = 'N'           #MOD-F20057 remark
                                #MOD-F20057 mark str
...
#---------------------end --------------------------------------
               ...
               #---------No.MOD-830194 end
                ...
                LET g_sfa.sfa01 =g_wo
               #CHI-C30128(1) -- add start --
                LET g_sfa.sfa012 = l_sfa012
                LET g_sfa.sfa013 = l_sfa013
               #CHI-C30128(1) -- add end --
                LET g_sfa.sfa02 =g_wotype
        ...
        CASE
         WHEN g_sfa.sfa26='1' AND g_sma.sma66='Y'	## UTE:先備舊料,再備新料
              ...
              IF l_s THEN       ##發生取代
                 ...
                 LET l_total2=l_unaloc         #20220729 mark   原應發量=取代量
                 #LET l_total2=l_total2-l_unaloc #20220729 modify 原應發量=原應發量-取代理
              ELSE 
                   LET g_sfa.sfa26='1'
        ...
        END CASE
         ...
         #FUN-910088 --add--end--
           UPDATE sfa_file SET sfa05 = l_total2,
                               ...
                               sfa25 = l_unaloc,          #20220729 mark
                              #sfa25 = l_total2,          #20220729 modify 未備料量=應發量
                               sfa26 = g_sfa.sfa26,
...
#        SELECT bmd03,bmd04,bmd07,ima262,ima27,ima08 #FUN-A20044
       #SELECT bmd03,bmd04,bmd07,0,ima27,ima08 #FUN-A20044          #20220802 mark
        SELECT DISTINCT bmd03,bmd04,bmd07,0,ima27,ima08 #FUN-A20044 #20220802 modify distinct
    ...
    ### LET l_unaloc=l_unaloc-l_qty
       #IF l_qoh >= l_unaloc THEN            #庫存量夠時   #MOD-A40136 mark
        ...
        ELSE
           ...
           LET l_unaloc=l_unaloc-l_qoh       #庫存不足繼續找取替代              #20200210 mark
          # LET l_unaloc=l_qty-l_qoh       #庫存不足繼續找取替代              #20200210 mark
        END IF
...
#MOD-A50131 add --begin
        ...
        #check 最少發料量
        #IF l_ima641 != 0 AND l_total < l_ima641 THEN                  #20200320 mark
         IF l_ima641 != 0 AND l_total < l_ima641 AND l_total !=0 THEN  #20200320 modify
            LET l_total = l_ima641
         END IF
        #IF l_ima64 != 0 THEN                                          #20200320 mark
         IF l_ima64 != 0 AND l_total !=0 THEN                          #20200320 modify
...
FUNCTION cralc_oeo(p_part,p_woq)
   ...
   IF g_sfb.sfb22 IS NULL AND g_sfb.sfb86 IS NULL THEN RETURN END IF
   ...
   ELSE
      LET l_sfb22 = g_sfb.sfb22
      LET l_sfb221= g_sfb.sfb221
   END IF

   IF l_sfb22 IS NULL THEN RETURN END IF
   #M012 180131 By TSD.Nic -----(E)
   DECLARE cralc_oeo_c CURSOR FOR
       SELECT * FROM oeo_file 
        #M012 180131 By TSD.Nic -----(S)
        #WHERE oeo01=g_sfb.sfb22 AND oeo03=g_sfb.sfb221
        WHERE oeo01=l_sfb22 AND oeo03=l_sfb221
          AND ta_oeo01 = g_sfb.sfb05
        #M012 180131 By TSD.Nic -----(E)
          AND oeo08='1'  #no.7168
...
#   LET g_sfa.sfa07 =0   #FUN-940008 mark
    #M012 180131 By TSD.Nic -----(S)
    #LET g_sfa.sfa08 =' '
    LET g_sfa.sfa08 =l_oeo.ta_oeo02
    #M012 180131 By TSD.Nic -----(E)
    LET g_sfa.sfa09 =0
    LET g_sfa.sfa11 ='N'
...
DEFINE l_brb06_07     LIKE brb_file.brb06     #FUN-FC0005 add 
    ...
    WHILE TRUE
        ...
        FOR l_i = 1 TO l_x
           ...
           #CALL cralc_rate(p_key,sr[l_i].brb03,p_total,sr[l_i].brb081,sr[l_i].brb08,sr[l_i].brb082,l_brb06,p_QPA) 
            #FUN-FC0005 mark end   --------------------------------------------
            #FUN-FC0005 add start --------------------------------------------
            LET l_brb06_07=sr[l_i].brb06/sr[l_i].brb07
            CALL cralc_rate(p_key,sr[l_i].brb03,p_total,sr[l_i].brb081,sr[l_i].brb08,sr[l_i].brb082,l_brb06_07,p_QPA,sr[l_i].brb06,sr[l_i].brb07) 
            #FUN-FC0005 add end   --------------------------------------------
            RETURNING l_total,l_QPA,l_ActualQPA
            LET l_total2=l_total
            LET l_sfa11='N'
...
#FUNCTION cralc_rate(p_bmb01,p_bmb03,p_total,p_bmb081,p_bmb08,p_bmb082,p_bmb06,p_QPA)                      #FUN-FC0005 mark
FUNCTION cralc_rate(p_bmb01,p_bmb03,p_total,p_bmb081,p_bmb08,p_bmb082,p_bmb06_07,p_QPA,p_bmb06,p_bmb07)    #FUN-FC0005 add
DEFINE  l_n         LIKE type_file.num5,
      ...
      #p_total      LIKE sfa_file.sfa05,     #MOD-E50001 mark
       p_total      LIKE type_file.num26_10, #MOD-E50001 add
...
DEFINE p_bmb06_07   LIKE bmb_file.bmb06    #FUN-FC0005 add
   ...
   IF g_sma.sma71='Y' AND g_sma.sma141='Y' THEN                
      ...
      FOR l_a=1 TO l_c
        ...
        #IF t_total>sr[l_a].bof05 THEN  #FUN-B60142
         IF p_total>sr[l_a].bof05 THEN  #FUN-B60142
           ...
           #LET sr1[l_a].total=l_bmb081+p_bmb06*(sr[l_a].bof05-sr[l_a].bof04+1)*(p_bmb08/100/(p_bmb082))*(1+sr[l_a].bof07/100)     #FUN-B60142 #FUN-FC0005 mark
            LET sr1[l_a].total=l_bmb081+p_bmb06/p_bmb07*(sr[l_a].bof05-sr[l_a].bof04+1)*(p_bmb08/100/(p_bmb082))*(1+sr[l_a].bof07/100)         #FUN-FC0005 add
            IF l_a=l_c THEN 
              ...
              #LET sr1[l_c+1].total=p_bmb06*t_total*(p_bmb08/100/(p_bmb082))*1               #FUN-B60142    #FUN-FC0005 mark
               LET sr1[l_c+1].total=p_bmb06/p_bmb07*t_total*(p_bmb08/100/(p_bmb082))*1                      #FUN-FC0005 add
            END IF 
         ELSE 
           ...
           #LET sr1[l_a].total=l_bmb081+p_bmb06*t_total*(p_bmb08/100/(p_bmb082))*(1+sr[l_a].bof07/100) #FUN-B60142  #FUN-FC0005 mark
            LET sr1[l_a].total=l_bmb081+p_bmb06/p_bmb07*t_total*(p_bmb08/100/(p_bmb082))*(1+sr[l_a].bof07/100)      #FUN-FC0005 add
            EXIT FOR 
         END IF 
      END FOR 
      LET l_total=0
      ...
      END FOR 
      IF l_total=0 THEN 
        #LET l_total=p_bmb081*1+p_bmb06*p_total*(p_bmb08/100/(p_bmb082))*1  #MOD-AC0334		 #MOD-AC0339   #FUN-FC0005 mark
         LET l_total=p_bmb081*1+p_bmb06/p_bmb07*p_total*(p_bmb08/100/(p_bmb082))*1                             #FUN-FC0005 add 
      END IF       
     #LET l_total=l_total+p_total*p_bmb06    #No.FUN-A60080  #FUN-FC0005 mark
      LET l_total=l_total+p_total*p_bmb06/p_bmb07            #FUN-FC0005 add 
      LET l_ActualQPA=l_total/p_total
     #LET l_QPA=p_bmb06*p_QPA                #No.FUN-A60080  $FUN-FC0005 mark
      LET l_QPA=p_bmb06/p_bmb07*p_QPA                        #FUN-FC0005 add 
   END IF 
   #损耗不分量计算
  #IF g_sma.sma71='Y' AND g_sma.sma141='N' THEN  #TQC-AB0125
   IF g_sma.sma71='Y' AND (g_sma.sma141<>'Y' OR cl_null(g_sma.sma141)) THEN #TQC-AB0125 
     ...
     #LET l_total=l_total+p_total*p_bmb06   #No.FUN-A60086
      #FUN-FC0005 mark end   --------------------------------
      #FUN-FC0005 add start --------------------------------
      LET l_QPA=p_bmb06/p_bmb07*p_QPA    
      LET l_total=p_bmb081*(1+l_bof06/100)+ p_bmb06/p_bmb07*p_total*(p_bmb08/100/(p_bmb082))*(1+l_bof07/100)  
      LET l_total=l_total+p_total*p_bmb06/p_bmb07   
      #FUN-FC0005 add end   --------------------------------
      LET l_ActualQPA=l_total/p_total
   END IF    
   #不考虑损耗
   IF g_sma.sma71='N' THEN 
     #LET l_QPA=p_bmb06 * p_QPA              #FUN-FC0005 mark
      LET l_QPA=p_bmb06/p_bmb07 * p_QPA      #FUN-FC0005 add
      LET l_ActualQPA=l_QPA
     #LET l_total=p_total*p_bmb06            #FUN-FC0005 mark
      LET l_total=p_total*p_bmb06/p_bmb07    #FUN-FC0005 add
   END IF 
...
#返回值 备料量，标准QPA,实际QPA
#FUNCTION cralc_eck_rate(p_sfb01,p_ecm03,p_ecm012,p_total,p_bmb081,p_bmb08,p_bmb082,p_bmb06,p_QPA)                       #FUN-FC0005 mark
FUNCTION cralc_eck_rate(p_sfb01,p_ecm03,p_ecm012,p_total,p_bmb081,p_bmb08,p_bmb082,p_bmb06_07,p_QPA,p_bmb06,p_bmb07)     #FUN-FC0005 add
DEFINE  l_n         LIKE type_file.num5,
        sr          DYNAMIC ARRAY OF RECORD 
...
DEFINE p_bmb06_07   LIKE bmb_file.bmb06      #FUN-FC0005 add
   ...
   IF g_sma.sma71='Y' AND g_sma.sma141='Y' THEN                
      ...
      FOR l_a=1 TO l_c
        ...
        #IF t_total>sr[l_a].eck05 THEN   #FUN-B60142
         IF p_total>sr[l_a].eck05 THEN   #FUN-B60142
           ...
           #LET sr1[l_a].total=l_bmb081+p_bmb06*(sr[l_a].eck05-sr[l_a].eck04+1)*(p_bmb08/100/(p_bmb082))*(1+sr[l_a].eck07/100) #FUN-B60142 #FUN-FC0005 mark
            LET sr1[l_a].total=l_bmb081+p_bmb06/p_bmb07*(sr[l_a].eck05-sr[l_a].eck04+1)*(p_bmb08/100/(p_bmb082))*(1+sr[l_a].eck07/100)     #FUN-FC0005 add
            IF l_a=l_c THEN 
              #LET sr1[l_c+1].total=p_bmb081*1+p_bmb06*t_total*(p_bmb08/100/(p_bmb082))*1  #MOD-AC0334	#MOD-AC0339 #FUN-B60142
              #LET sr1[l_c+1].total=p_bmb06*t_total*(p_bmb08/100/(p_bmb082))*1             #FUN-B60142  #FUN-FC0005 mark
               LET sr1[l_c+1].total=p_bmb06/p_bmb07*t_total*(p_bmb08/100/(p_bmb082))*1                  #FUN-FC0005 add             
         ...
         ELSE 
           ...
           #LET sr1[l_a].total=l_bmb081+p_bmb06*t_total*(p_bmb08/100/(p_bmb082))*(1+sr[l_a].eck07/100)                        #FUN-B60142  #FUN-FC0005 mark
            LET sr1[l_a].total=l_bmb081+p_bmb06/p_bmb07*t_total*(p_bmb08/100/(p_bmb082))*(1+sr[l_a].eck07/100)                             #FUN-FC0005 add 
            EXIT FOR 
         END IF 
      END FOR 
      LET l_total=0
      ...
      END FOR 
      IF l_total=0 THEN 
        #LET l_total=p_bmb081*1+p_bmb06*p_total*(p_bmb08/100/(p_bmb082))*1 #FUN-A60087 #MOD-AC0334	 #MOD-AC0339   #FUN-FC0005 mark
         LET l_total=p_bmb081*1+p_bmb06/p_bmb07*p_total*(p_bmb08/100/(p_bmb082))*1                                     #FUN-FC0005 add 
      END IF 
     #LET l_total=l_total+p_total*p_bmb06  #No.FUN-A60080  #FUN-FC0005 mark
      LET l_total=l_total+p_total*p_bmb06/p_bmb07          #FUN-FC0005 add 
  ...
  #IF g_sma.sma71='Y' AND g_sma.sma141='N' THEN #TQC-AB0125
   IF g_sma.sma71='Y' AND (g_sma.sma141<>'Y' OR cl_null(g_sma.sma141)) THEN #TQC-AB0125 
     ...
     #LET l_total=p_bmb081*(1+l_eck06/100)+ p_bmb06*p_total*(p_bmb08/100/(p_bmb082))*(1+p_bmb08*l_eck07/p_bmb082)/100     #MOD-AC0334  #MOD-AC0339   #FUN-FC0005 mark
      LET l_total=p_bmb081*(1+l_eck06/100)+ p_bmb06/p_bmb07*p_total*(p_bmb08/100/(p_bmb082))*(1+p_bmb08*l_eck07/p_bmb082)/100                        #FUN-FC0005 add 
     #LET l_total=l_total+p_total*p_bmb06  #No.FUN-A60080  #FUN-FC0005 mark
      LET l_total=l_total+p_total*p_bmb06/p_bmb07          #FUN-FC0005 add 
      LET l_ActualQPA=l_total/p_total
   END IF    
