...
#------------No.TQC-610089 modify
  #LET tm.more  = ARG_VAL(14)
   ...
   LET tm.oea00 = ARG_VAL(14)    #FUN-EA0031 modify ARG_VAL(15) -> ARG_VAL(16)
   #MOD-CC0246 -- add end --     #參數順序搬移
   #No.FUN-570264 --start--
   LET g_rep_user = ARG_VAL(15)                   #MOD-CC0246 modify ARG_VAL(15) -> ARG_VAL(16)   #FUN-EA0031 modify ARG_VAL(16) -> ARG_VAL(17)
   LET g_rep_clas = ARG_VAL(16)                   #MOD-CC0246 modify ARG_VAL(16) -> ARG_VAL(17)   #FUN-EA0031 modify ARG_VAL(17) -> ARG_VAL(18)
   LET g_template = ARG_VAL(17)                   #MOD-CC0246 modify ARG_VAL(17) -> ARG_VAL(18)   #FUN-EA0031 modify ARG_VAL(18) -> ARG_VAL(19)
   LET g_rpt_name = ARG_VAL(18)  #No.FUN-7C0078   #MOD-CC0246 modify ARG_VAL(18) -> ARG_VAL(19)   #FUN-EA0031 modify ARG_VAL(19) -> ARG_VAL(20)
   #No.FUN-570264 ---end---
#------------No.TQC-610089 end
...
DEFINE lc_qbe_sn      LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   OPEN WINDOW r410_w AT p_row,p_col WITH FORM "csf/42f/csfr999"
   ...
   WHILE TRUE
    ...
    #  END IF
      ...
      INPUT BY NAME tm.c,tm.bdate,tm.edate,                                    #20220317 #20250324
                    tm.a,tm.b,tm.more,tm.oea00,tm.x,tm.v,tm.d  #No.MOD-7B0154  #FUN-850142 Add tm.x  #TQC-940009 add tm.v   #No.FUN-960004 add tm.oea00  #FUN-EA0031 add tm.d
            WITHOUT DEFAULTS
         #No.FUN-580031 --start--
         BEFORE INPUT
...
DEFINE l_wip            LIKE type_file.num10            #20231129 WIP量
   ...
   IF tm.c='3' THEN
      LET tm.wc2 = tm.wc2 CLIPPED," AND sfb86=' ' "
   END IF

   ##----備料明細 (S)
   IF tm.c <> '2' THEN
      LET l_sql = " SELECT '','','',oea01,oea02,oea03, A.occ02, oea04, oea10, oea044,",
                  " B.occ02, A.occ21, oea915,oea916,'','',oea42, oeahold,oeaconf,oea14, ",
                  ...
                  " LEFT JOIN oeb_file ON oeb01 = substr(ta_sfb01,1,15) AND LPAD(oeb03,3,'0')=substr(ta_sfb01,16,18) ",
                  ...
                  " WHERE ima01=sfb05 AND sfb87 = 'Y'  ",
                  "   AND NOT (sfb04 ='8' AND sfb081 = 0) ",   #工單結案且發料套量為0不呈現
                  "   AND sfb13 BETWEEN '",tm.bdate,"' AND '",tm.edate,"' ",
                  "   AND ", tm.wc CLIPPED ,   
                  "   AND ", tm.wc2 CLIPPED ,
                  " UNION ",
                  " SELECT '2','','',  inb01, ina03, '', '', '', '', '',",
                  "        '', '', '','', '', '',  '', '','','',",
                  "        '',      inb03,sfb05, ima02,      '', '', inb16, 0, inaud14,     ",
                  "       inaud13, '', '','', inaud14, '',       '',0,'',0,'',0,sfb01,ima021  ",  #20250321 modify
                  "  FROM ima_file,sfb_file ",
                  "  INNER JOIN inb_file ON sfb91=substr(ta_sfb01,1,15) AND LPAD(inb03,3,'0')=substr(ta_sfb01,16,18)  ",         
                  "  INNER JOIN ina_file ON inb01=ina01  ",
                  " WHERE sfb05 = ima01 ",
...
#   FINISH REPORT r410_rep
      ...
      END IF
      LET g_str=tm.wc ,";",g_sma115,";",g_sma.sma116
