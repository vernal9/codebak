...
#
# Pattern name...: cbmi604.4gl
# Descriptions...: 元件取替代資料維護
# Date & Author..: 20190709 By momo
# Modify,........: NO:2106256555 20210701 by momo 增加群組替代選項
# Modify.........: NO:22100020   20221020 By momo 調整修改與複製 BUG
...
MAIN
    ...
    LET l_table = cl_prt_temptable('cbmi604',g_sql) CLIPPED
    ...
    LET g_tc_bmd02 = NULL                     #清除鍵值
    LET g_tc_bmd01_t = NULL
    LET g_tc_bmd08_t = NULL
    LET g_tc_bmd02_t = NULL
    ...
    IF g_argv3=' ' THEN LET g_argv3='' ELSE LET g_tc_bmd02=g_argv3 END IF
 
    OPEN WINDOW i604_w WITH FORM "cbm/42f/cbmi604"
    ...
    IF NOT cl_null(g_argv1) AND g_row_count = 0  THEN CALL i604_a() END IF 
...
FUNCTION i604_cs()
   ...
   INITIALIZE g_tc_bmd02 TO NULL    
      ...
      ELSE
	LET g_wc=" tc_bmd01='",g_argv1,"'"
    END IF
    LET g_sql="SELECT DISTINCT tc_bmd01,tc_bmd08,tc_bmd02 FROM tc_bmd_file ", 
               " WHERE tc_bmd02 NOT IN ('3','9') AND ", g_wc CLIPPED,                      #20201109
    ...
    LET g_sql="SELECT DISTINCT tc_bmd01,tc_bmd08,tc_bmd02 FROM tc_bmd_file WHERE tc_bmd02 NOT IN ('3','9') AND ",  #20201109
               g_wc CLIPPED
...
DEFINE  l_num       LIKE type_file.num5      
	...
	BEFORE FIELD tc_bmd01  # 是否可以修改 key
	 #   IF g_chkey = 'N' AND p_cmd = 'u' THEN RETURN END IF
         IF p_cmd = 'u' THEN
           #CALL cl_set_comp_entry("tc_bmd01,tc_bmd02,tc_bmd08","FALSE") #20210701 mark
            CALL cl_set_comp_entry("tc_bmd01,tc_bmd08","FALSE")          #20210701 modify
         END IF
	    ...
	    IF NOT cl_null(g_tc_bmd08) THEN
                IF g_tc_bmd08 IS NOT NULL THEN
                    CALL i604_tc_bmd08('a')
                    IF NOT cl_null(g_errno) THEN
                       	CALL cl_err(g_tc_bmd08,g_errno,0)
                       	LET g_tc_bmd08 = g_tc_bmd08_t
...
FUNCTION i604_tc_bmd08(p_cmd)
    DEFINE p_cmd     LIKE type_file.chr1,          #No.FUN-680096 VARCHAR(1)
           l_ima1007 LIKE ima_file.ima1007,
           l_imaacti LIKE ima_file.imaacti
 
    LET g_errno = ' '
    SELECT distinct ima1007
           INTO l_ima1007
           FROM ima_file WHERE ima1007 = g_tc_bmd08
    CASE WHEN SQLCA.SQLCODE = 100  LET g_errno = 'mfg0002'
                            LET l_ima1007= NULL
         WHEN l_imaacti='N' LET g_errno = '9028'
...
FUNCTION i604_q()
  DEFINE l_tc_bmd01  LIKE tc_bmd_file.tc_bmd01,
    ...
    IF SQLCA.sqlcode THEN                         #有問題
        ...
        INITIALIZE g_tc_bmd02 TO NULL
    ELSE
        FOREACH i604_count INTO l_tc_bmd01,l_tc_bmd08,l_tc_bmd02
...
DEFINE
  ...
  DEFINE l_tc_bmd01  LIKE tc_bmd_file.tc_bmd01,
    ...
    MESSAGE ""
    CASE p_flag
        WHEN 'N' FETCH NEXT     i604_bcs INTO g_tc_bmd01,g_tc_bmd08,g_tc_bmd02 
        WHEN 'P' FETCH PREVIOUS i604_bcs INTO g_tc_bmd01,g_tc_bmd08,g_tc_bmd02
        WHEN 'F' FETCH FIRST    i604_bcs INTO g_tc_bmd01,g_tc_bmd08,g_tc_bmd02
        WHEN 'L' FETCH LAST     i604_bcs INTO g_tc_bmd01,g_tc_bmd08,g_tc_bmd02
        WHEN '/'
      ...
      ON ACTION controlg      
         CALL cl_cmdask()     
            ...
            END IF
            LET g_no_ask = FALSE
            FETCH ABSOLUTE g_jump i604_bcs INTO g_tc_bmd01,g_tc_bmd08,g_tc_bmd02
    END CASE
        ...
        BEFORE ROW
            ...
            IF g_rec_b >= l_ac THEN
               ...
               LET g_tc_bmd04_o = g_tc_bmd[l_ac].tc_bmd04  #BACKUP
                BEGIN WORK
                OPEN i604_bcl USING g_tc_bmd01,g_tc_bmd08,g_tc_bmd02,g_tc_bmd_t.tc_bmd03
                IF STATUS THEN
                    CALL cl_err("OPEN i604_bcl:", STATUS, 1)
                    LET l_lock_sw = "Y"
                ELSE
                    FETCH i604_bcl INTO g_tc_bmd[l_ac].*
                    IF SQLCA.sqlcode THEN
        ...
        AFTER FIELD tc_bmd03                        #check 序號是否重複
         ...
         AFTER FIELD tc_bmd04
             IF NOT cl_null(g_tc_bmd[l_ac].tc_bmd04) THEN
               ...
               #檢查替代料，是否與主件料號相同
                IF g_tc_bmd[l_ac].tc_bmd04 = g_tc_bmd08 THEN
                   CALL cl_err(g_tc_bmd[l_ac].tc_bmd04,'cbm-126',1)
                   NEXT FIELD tc_bmd04
                END IF
...
DEFINE
    ...
    DECLARE tc_bmd_cs CURSOR FOR i604_prepare2
    CALL g_tc_bmd.clear()
    LET g_cnt = 1
    LET g_rec_b=0
...
FUNCTION i604_bp(p_ud)
   ...
   DISPLAY ARRAY g_tc_bmd TO s_tc_bmd.* ATTRIBUTE(COUNT=g_rec_b,UNBUFFERED)
	...
	ACCEPT DISPLAY                  
      ...
      ON ACTION cus_sub
         LET g_action_choice="cus_sub"
         EXIT DISPLAY
 
      ON ACTION cooi103
         LET g_action_choice="cooi103"
         EXIT DISPLAY
...
DEFINE
    ...
    CALL cl_set_comp_entry("tc_bmd08",TRUE) #20221020 

    INPUT l_newno2 FROM tc_bmd08

        BEFORE FIELD tc_bmd08
...
DEFINE l_tc_bmd02        LIKE tc_bmd_file.tc_bmd02
   ...
   FOREACH i604_bcs INTO l_tc_bmd01,l_tc_bmd08,l_tc_bmd02     
      IF SQLCA.sqlcode THEN
         CALL cl_err('foreach item_cur',SQLCA.sqlcode,1)
         CONTINUE FOREACH
      END IF
      SELECT tc_bmd01,a.ima02,a.ima021,a.ima25,a.ima08,tc_bmd02,tc_bmd08,tc_bmdnote
        INTO g_tc_bmd_1[l_cnt].*
        FROM tc_bmd_file LEFT JOIN ima_file a ON tc_bmd01 = a.ima01
